...
public class SQLite {
	...
	public Connection connect(String dbConnectionString) {
		try {
			...
			LOGGER.severe(LogStackTrace.get(e));
			return null;
		} catch (Exception ex) {
			LOGGER.severe(LogStackTrace.get(ex));
			return null;
		}
	}
	...
	public synchronized Integer ddlQuery(String query) {
		...
		try {
			...
			if (err.contains("SQLITE_BUSY")) {
				...
				LOGGER.severe(StringOps.append("SQL Lite exception....\n", err));
		} catch (Exception e) {
			LOGGER.severe(StringOps.append("SQL exception....\n", LogStackTrace.get(e)));
		} finally {
